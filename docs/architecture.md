...
GitHub bulk import opens from the Projects screen as a focused dialog. It lists accessible repositories from the configured GitHub users or organizations and lets the user select which ones become tracked projects. Import defaults lifecycle state from hosted facts such as visibility and archive state, while still allowing an explicit lifecycle state during import and optional checkout after import.
