...
async function runJob(
  ...
  if (requiresManualApproval(loaded, job)) {
    return planManualApprovalJob(loaded, job, slot, startedAt, catchUp, eligibleProjects);
  }
...
async function runCodeRabbitRecommendationJob(
  ...
  return {
    ...
    message: `created ${result.approvals.length} CodeRabbit approval candidate(s)`,
    startedAt,
    finishedAt: new Date().toISOString(),
  };
}
...
async function runIssueRecommendationJob(
  ...
  return {
    ...
    status: "succeeded",
    reportPath: result.reportPath,
    message: `created ${result.approvals.length} issue approval candidate(s)`,
    startedAt,
    finishedAt: new Date().toISOString(),
  };
}

function requiresManualApproval(loaded: LoadedConfig, job: AutomationJob): boolean {
  return (
    loaded.config.automation.implementation_gate === "manual" && job.mode === "patch-candidate"
  );
}

async function planManualApprovalJob(
  loaded: LoadedConfig,
  job: AutomationJob,
  slot: AutomationDueSlot,
  startedAt: string,
  ...
  const reportPath = await writeReport(
    loaded,
    outputPath(job, slot),
    renderManualApprovalReport(job, slot, catchUp, eligibleProjects),
  );
  const approval = await upsertApproval(loaded, {
    id: automationApprovalId(job, slot),
    title: `${job.id} ${slot.slotId}`,
    kind: "automation",
    reportPath,
    reason: `${job.mode} requires manual approval before implementation work runs.`,
    metadata: {
      jobId: job.id,
      slotId: slot.slotId,
      mode: job.mode,
  ...
  return {
    ...
    status: "planned",
    reportPath,
    message: `manual approval required: ${approval.id}`,
    startedAt,
    finishedAt: new Date().toISOString(),
  };
}

function automationApprovalId(job: AutomationJob, slot: AutomationDueSlot): string {
  return `automation:${job.id}:${slot.slotId}`;
}
...
function renderManualApprovalReport(
  job: AutomationJob,
  slot: AutomationDueSlot,
  catchUp: boolean,
  eligibleProjects: readonly Project[] | undefined,
): string {
  return `${renderAutomationReportHeader(job, slot, catchUp, "manual approval required", eligibleProjects)}
This job is gated by Herakles automation. Approve the generated automation candidate before any implementation or patch work is allowed to run.
`;
}
