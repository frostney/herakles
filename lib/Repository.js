...
Repository.prototype.sync = function sync() {
  ...
  return mkdirp(this.baseDirectory).then(function gitThen() {
    ...
    return nodegit.Repository.open(self.baseDirectory).then(function fetchThen(repo) {
      repository = repo;

      return repository.fetchAll(opts.fetchOpts);
    }).then(function mergeThen() {
      // TODO: Not all remotes are called origin
      repository.mergeBranches('master', 'origin/master');
    }).catch(function clone() {
