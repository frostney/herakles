...
var Promise =
  typeof Promise === "undefined" ? require("pinkie-promise") : Promise;

var events = require("./events");

// TODO: Shorthands should be handled more dynamically
const expandShorthand = (shorthand: string): string => {
  const removeAbbreviation = (short: string, abbreviation: string): string => {
    return short.split(abbreviation)[1];
  };

  if (shorthand.indexOf("github:") === 0) {
    return (
      "git@github.com:" + removeAbbreviation(shorthand, "github:") + ".git"
    );
  }
  ...
  if (shorthand.indexOf("bitbucket:") === 0) {
    return (
      "git@bitbucket.org:" +
      removeAbbreviation(shorthand, "bitbucket:") +
      ".git"
    );
  }
  ...
  if (shorthand.indexOf("gitlab:") === 0) {
    return (
      "git@gitlab.com:" + removeAbbreviation(shorthand, "gitlab:") + ".git"
    );
  }
...
class Repository {
  ...
  pull(repository: Repository, opts: any) {
    return repository
      .fetchAll(opts.fetchOpts)
      .then(function remotesThen() {
        return Promise.all([
  
  }

  push(repository: Repository, opts: any) {
...
Repository.prototype.push = function push(repository, opts) {
  return Promise.all([
    ...
    return repository.getRemote(defaultRemote).then(function pushDirectly(r) {
      return r.push(
        ["refs/heads/" + shorthand + ":refs/heads/" + shorthand],
        opts.fetchOpts
      );
    });
  });
...
Repository.prototype.sync = function sync() {
  ...
  return mkdirp(this.baseDirectory).then(function gitThen() {
    if (!repositoryExists(self.baseDirectory)) {
      return nodegit.Clone.clone(self.location, self.baseDirectory, opts).then(
        function emitEvent() {
    ...
    return nodegit.Repository.open(self.baseDirectory).then(function openThen(
      repository
    ) {
