...
Herakles.prototype.addGroup = function addGroup(name) {
  if (!Object.hasOwnProperty.call(this.children, name)) {
    ...
    this.children[name].root = this;
    this.children[name].name = name;
    this.children[name].root.groupCount++;
...
/*
  TODO: We could probably improve concurrency here.
  Unfortunately nodegit causes a `malloc` if cloning happens in two
  adjecent sub-folders :(
*/
Herakles.prototype.sync = function sync() {
  var initialPromise = Promise.resolve();

  events.emit('sync:start');

  return this.toArray().reduce(function reducer(syncPromise, child) {
    return syncPromise = syncPromise.then(function syncThen() {
      return child.sync();
    });
  }, initialPromise).then(function emitEvent() {
    events.emit('sync:end');
  });
};

Herakles.events = events;
